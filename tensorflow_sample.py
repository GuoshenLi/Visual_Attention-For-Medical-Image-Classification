...
def get_resampled_images(image, saliency, batch_size, src_size, dst_size, padding_size = 30):
	...
	pixel_wise_sample_grids, src_grids = generate_pixel_grid(padded_saliency, batch_size, src_size, dst_size) # (8, 128, 128, 2)
...
def compute_saliency(s_map):

    s_map_min = tf.reduce_min(s_map, axis=[1, 2, 3], keepdims=True)
    s_map_max = tf.reduce_max(s_map, axis=[1, 2, 3], keepdims=True)
    s_map = tf.div(s_map - s_map_min + 1e-8, s_map_max - s_map_min + 1e-8)

    saliency_map = tf.tile(s_map, (1, 1, 1, 3))
    saliency_map = tf.image.resize_images(saliency_map, (512, 512))  # (8, 128, 128, 3)

    return saliency_map

img = cv2.imread('kris.png')
img = cv2.resize(img, (512, 512))
img = img[:,:,::-1] / 225.
print(img.shape)
image = tf.cast(tf.convert_to_tensor(img)[tf.newaxis,:], tf.float32)

saliency = np.zeros((1, 31, 31, 1))
# simple saliency map
saliency[:,10:20,10:20,:] = 1

saliency = tf.cast(tf.convert_to_tensor(saliency), tf.float32)
saliency = tf.reshape(tf.nn.softmax(tf.reshape(saliency, (-1, 31 * 31))), (-1, 31, 31, 1))


resampled_image = get_resampled_images(image, saliency, batch_size = 1, src_size = 512, dst_size = 512, padding_size = 30)
saliency = compute_saliency(saliency)
...
with tf.Session() as sess:
    ...
    saliency_ = sess.run(saliency)
    resampled_image_ = sess.run(resampled_image)
    plt.imshow(saliency_[0,:,:,:])
    plt.show()

    plt.imshow(resampled_image_[0,:,:,:])
    plt.show()