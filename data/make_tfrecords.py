...
def get_tfdata_path(tfrecords_path, split_name):
    ...
    tfrecords_filename = os.path.join(tfrecords_path, split_name)
...
def to_tfrecords(txt_path, tf_path, split_name):#
    with open(txt_path, 'r') as f:
        ...
        write_tf_data(txt_list, tf_path, split_name)


def write_tf_data(txt_list, tfrecords_path, split_name):
    ...
    with tf.python_io.TFRecordWriter(tfrecords_filename) as tfrecords_writer:
        for i in range(0, len(txt_list)):
            ...
            label = int(txt_list[i].split(',')[1])

            data = cv2.imread(rpath)
            #data = data[32:544,32:544, :]
            data = cv2.resize(data, (320, 320))
            data1 = np.zeros([320, 320, 3],dtype = np.uint8)
...
to_tfrecords(txt_path, tf_path, split_name)
